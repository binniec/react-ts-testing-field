...
export default function Transactions() {
    ...
    function updateItemDiscount(itemIndex: number, event: ChangeEvent): void {
        ...
        if (element.valueAsNumber < 0 || element.valueAsNumber > 99) {
            return;
        }
        ...
        setList(list.map((item, index) => {
            ...
            if (index === itemIndex) {
                return { ...item, discount: Math.floor(element.valueAsNumber) };
            } else if (element.valueAsNumber < 0) {
    ...
    function updateTransactionDiscount(event: ChangeEvent): void {
        ...
        setTransactionDiscount(element.valueAsNumber);
    }
    ...
    function getSubtotal(): number {
        ...
        list.map(item => {
            ...
            // If the item as an entered discount, it will override the transaction wide discount.
            if (item.discount > 0) {
                subtotal += ((item.price - (item.price * item.discount / 100)) * item.quantity);
            } else {
                subtotal += ((item.price - (item.price * (transactionDiscount ? transactionDiscount : 0) / 100)) * item.quantity);
            }
                    ...
                    <div>
                        ...
                        <span>
                            <input
                                ...
                                onChange={e => updateTransactionDiscount(e)} />
                        </span>
                    </div>
